...
-- CREATE ALGORITHM=UNDEFINED DEFINER=`if0_40141531`@`sql212.infinityfree.com` SQL SECURITY DEFINER VIEW `order_status_summary`  AS SELECT `o`.`id` AS `id`, `o`.`ref_code` AS `ref_code`, `o`.`client_id` AS `client_id`, concat(`c`.`lastname`,', ',`c`.`firstname`,' ',coalesce(`c`.`middlename`,'')) AS `client_name`, `o`.`total_amount` AS `total_amount`, `o`.`status` AS `status`, CASE WHEN `o`.`status` = 0 THEN 'Pending' WHEN `o`.`status` = 1 THEN 'Approved Order' WHEN `o`.`status` = 2 THEN 'For Delivery' WHEN `o`.`status` = 3 THEN 'On the Way' WHEN `o`.`status` = 4 THEN 'Delivered' WHEN `o`.`status` = 6 THEN 'Claimed' WHEN `o`.`status` = 5 THEN 'Cancelled' ELSE 'Unknown' END AS `status_text`, `o`.`date_created` AS `date_created`, `o`.`date_updated` AS `date_updated` FROM (`order_list` `o` join `client_list` `c` on(`o`.`client_id` = `c`.`id`)) WHERE `c`.`delete_flag` = 0 ;
