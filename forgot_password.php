...
<body class="hold-transition login-page">
  ...
  <style>
      ...
      .card.card-outline.card-primary{ backdrop-filter: blur(2px); background: rgba(255,255,255,0.94); border: 1px solid var(--bs-primary); }
      .card.card-outline.card-primary .card-header{ background: var(--bs-primary); color: #fff; }
      #token-hint.alert{ border: 1px solid var(--bs-primary); background: rgba(220,53,69,0.06); color: #7a1b23; }
      #token-hint a.btn{ border-color: var(--bs-primary); color: var(--bs-primary); }
      #token-hint a.btn:hover{ background: var(--bs-primary); color:#fff; }
  </style>
  <div class="login-box">
    ...
    <div class="card card-outline card-primary">
      ...
      <div class="card-body">
        ...
        <form id="forgot-frm" method="post">
          ...
          <div class="alert alert-primary small" id="token-hint" style="display:none"></div>
          <div class="row align-items-center">
            ...
            <div class="col-12 col-sm-6 pl-sm-1 mb-2">
              <button type="submit" class="btn btn-primary btn-sm btn-flat btn-block"><i class="fa fa-paper-plane mr-1"></i> Send Link</button>
            </div>
          </div>
        </form>
      </div>
      ...
      $('#forgot-frm').submit(function(e){
        ...
        $.ajax({
          ...
          success: function(resp){
            ...
            if(link || token){
              var html = '';
              if(link){ html += '<div class="mb-1"><span class="fw-bold me-1">Reset Link:</span> <a class="btn btn-sm btn-outline-primary" href="'+link+'"><i class="fa fa-link me-1"></i>Open Link</a></div>'; }
              if(token){ html += '<div><span class="fw-bold me-1">Token:</span> <span class="badge bg-light text-primary border border-primary">'+token+'</span></div>'; }
